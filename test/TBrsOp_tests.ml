...
let test_parexplore_ss_1 _ =
    ...
    in
        ...
        in
            ...
            in
                let tl,ss,_ = TBrsOp.parexplore_ss ~s0 ~rules:[react] ~max_steps:10
                in
                    List.iteri
                        (
                            fun _ (t,ii,ri) -> 
                                let init_state_according_to_index,_ = List.find ( fun (_,i) -> if i = ii then true else false ) ss 
                                and res_state_according_to_index,_ = List.find ( fun (_,i) -> if i = ri then true else false ) ss
                                in
...
let test_parexplore_ss_2 _ =
    ...
    in
        ...
        in
            let react = TBrsOp.parse_react "yolo" ~lhs ~rhs ~f_rnm ~f_sm:None
            in
                let tl,ss,_ = TBrsOp.parexplore_ss ~s0 ~rules:[react] ~max_steps:4
                in
                    List.iteri
                        (
                            fun i (t,ii,ri) -> 
